...
public class CPInstance {
  ...
  public void solve() {
    try {
      ...
      List<IloNumVar[]> shiftSchedule = new ArrayList<>();
      List<IloNumVar[]> durationSchedule = new ArrayList<>();

      for (int i = 0; i < this.numEmployees; i++) {
        shiftSchedule.add(new IloNumVar[this.numDays]);
        IloNumVar[] x = shiftSchedule.get(i);
        for (int j = 0; j < this.numDays; j++) {
          x[j] = cp.numVar(0, 3);
        }

        durationSchedule.add(new IloNumVar[this.numDays]);
        IloNumVar[] y = durationSchedule.get(i);
        for (int j = 0; j < this.numDays; j++) {
          y[j] = cp.numVar(0, 8);
        }
      }
    ...
    public static void main (String[]args){
      String test_arg = "input/7_14.sched";
      CPInstance x = new CPInstance(test_arg);
      x.solve();
    }
